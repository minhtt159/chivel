...
def _validate_node(node: dict, node_cidr: str, distribution: str) -> None:
    ...
    if not node.get("talos", {}).get("disk_device") and distribution in ["talos"]:
...
@required(
    "bootstrap_cluster_cidr", "bootstrap_service_cidr", "bootstrap_advanced_flags"
)
def validate_cluster_cidrs(
    cluster_cidr: str, service_cidr: str, advanced_flags: dict, **_
) -> None:
    dual_stack_ipv4_first = advanced_flags.get("dual_stack_ipv4_first", False)

    if cluster_cidr == service_cidr:
        raise ValueError(
            f"Cluster CIDR {cluster_cidr} is the same as service CIDR {service_cidr}"
        )
...
def validate_cloudflare(
    domain: str, token: str, account_tag: str, tunnel_secret: str, tunnel_id: str, **_
) -> None:
    try:
        ...
        if not zones:
            raise ValueError(
                f"Cloudflare domain {domain} not found or token does not have access to it"
            )
    ...
    try:
        request = requests.get(
            f"https://api.cloudflare.com/client/v4/accounts/{account_tag}/cfd_tunnel/{tunnel_id}",
            headers={"Authorization": f"Bearer {token}"},
        )
        if request.status_code != 200:
            raise ValueError(
                f"Cloudflare tunnel for {account_tag} not found or token does not have access to it"
            )
...
def validate_host_network(
    node_cidr: str,
    api_addr: str,
    gateway_addr: str,
    external_ingress_addr: str,
    internal_ingress_addr: str,
    **_,
) -> None:
    ...
    if netaddr.IPAddress(api_addr) not in node_cidr:
        raise ValueError(
            f"Kubernetes API address {api_addr} is not in the node CIDR {node_cidr}"
        )
    if netaddr.IPAddress(gateway_addr) not in node_cidr:
        raise ValueError(
            f"Kubernetes gateway address {gateway_addr} is not in the node CIDR {node_cidr}"
        )
